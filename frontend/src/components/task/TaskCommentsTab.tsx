...
export const TaskCommentsTab: React.FC<TaskCommentsTabProps> = ({ 
  ...
  // Work-focused emoji reactions for professional communication
  const emojis = [
    '👍', // Approve/Agree - Basic approval and agreement
    '✅', // Done/Completed - Task completion and checkoffs
    '💡', // Great Idea - Innovation and smart suggestions
    '👏', // Well Done - Recognition and applause
    '🎯'  // Goal Achieved - Target and milestone completion
  ]
...
const CommentCard: React.FC<{
  ...
  return (
    <div className={`rounded-lg p-4 shadow-sm hover:shadow-md transition-all duration-200 ${getCommentBorder()}`}>
      ...
      {/* Reactions will now be placed below with Reply and React buttons */}
        ...
        <div className="flex items-center flex-wrap gap-3 text-sm text-gray-500">
          ...
          <div className="relative">
            <button
              ...
              className={`group flex items-center space-x-1.5 text-sm px-3 py-1.5 rounded-lg transition-all duration-300 border font-medium shadow-sm hover:shadow-lg relative overflow-hidden ${
                showEmojiPicker === comment.id 
                  ? 'text-blue-700 bg-gradient-to-r from-blue-100 to-indigo-100 border-blue-400 shadow-blue-100' 
                  : 'text-gray-600 hover:text-blue-600 bg-white hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 border-gray-200 hover:border-blue-300'
              }`}
              title="Add reaction"
...
// Enhanced Emoji Picker Portal Component
const EmojiPickerPortal: React.FC<{
  ...
  return createPortal(
    <>
      ...
      <div 
        ...
        onClick={onClose}
      />
      {/* Enhanced Single Row Emoji Picker - Now properly positioned */}
      <div 
        className={`fixed bg-white border border-gray-200 rounded-xl shadow-2xl z-[99999] p-4 min-w-[400px] max-w-[440px] ${
          position.placement === 'top' ? 'animate-in fade-in slide-in-from-bottom-2' : 'animate-in fade-in slide-in-from-top-2'
        } duration-200`}
        style={{ 
          left: `${position.x}px`,
          top: `${position.y}px`,
          ...
          maxWidth: 'min(440px, calc(100vw - 20px))',
