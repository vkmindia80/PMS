      ...
      <div className="bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 rounded-xl p-6 mb-6 border border-blue-200">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
          <div className="text-center bg-white rounded-lg p-3 shadow-sm border border-gray-100">
            <div className="text-2xl font-bold text-blue-600">{allComments.length}</div>
...
const CommentCard: React.FC<{
  ...
  return (
    <div className={`rounded-lg p-4 shadow-sm hover:shadow-md transition-all duration-200 ${getCommentBorder()}`}>
      ...
      {/* Enhanced Reactions Section - Moved to top with better sizing and contrast */}
      {comment.reactions && comment.reactions.length > 0 && (
        <div className="mb-4 pl-13">
          <div className="flex items-center justify-between">
                ...
                return Object.entries(reactionGroups).map(([emoji, data]) => {
                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex items-center space-x-2 px-4 py-2.5 rounded-full font-semibold transition-all duration-200 hover:scale-105 active:scale-95 border-2 shadow-sm ${
                        userHasReacted 
                          ? 'bg-blue-100 border-blue-400 text-blue-800 hover:bg-blue-200 hover:border-blue-500 shadow-blue-200/50' 
                          : 'bg-gray-50 border-gray-300 text-gray-700 hover:bg-gray-100 hover:border-gray-400 hover:shadow-md'
                      }`}
                    ...
                    >
                      <span className="text-xl" role="img" aria-label={`Reaction ${emoji}`}>{emoji}</span>
                      <span className={`text-sm font-bold min-w-[20px] text-center ${userHasReacted ? 'text-blue-900' : 'text-gray-800'}`}>
                        {data.count}
                      </span>
                    </button>
                  )
            ...
            {/* Add Reaction Button - More prominent version */}
            <div className="relative">
              <button
                ref={reactButtonRef}
                onClick={handleEmojiPickerToggle}
                className="flex items-center space-x-2 text-sm font-semibold text-gray-600 hover:text-blue-700 px-4 py-2.5 rounded-full hover:bg-blue-50 transition-all duration-200 border-2 border-gray-200 hover:border-blue-300 hover:shadow-md"
                title="Add reaction"
              ...
              {showEmojiPicker === comment.id && (
                <>
                  ...
                  />
                  {/* Emoji picker with enhanced sizing */}
                  <div className={`absolute ${pickerPosition === 'left' ? 'right-full mr-2' : 'right-0'} top-12 bg-white border-2 border-gray-300 rounded-xl shadow-2xl z-[9999] p-5 min-w-[320px] max-w-[380px]`}>
                    ...
                    <div className="grid grid-cols-6 gap-3">
                      {emojis.map((emoji) => (
                        <button
                          ...
                          className="p-4 hover:bg-blue-50 rounded-xl text-3xl transition-all duration-200 hover:scale-110 transform active:scale-95 focus:ring-3 focus:ring-blue-400 focus:outline-none border-2 border-transparent hover:border-blue-200 hover:shadow-md"
      ...
      {/* Add Reaction Button for comments without existing reactions */}
      {(!comment.reactions || comment.reactions.length === 0) && (
        <div className="mb-4 pl-13">
          <div className="relative">
            <button
              ref={reactButtonRef}
              onClick={handleEmojiPickerToggle}
              className="flex items-center space-x-2 text-sm font-medium text-gray-500 hover:text-blue-700 px-3 py-2 rounded-lg hover:bg-blue-50 transition-all duration-200 border border-gray-200 hover:border-blue-300"
              title="Add the first reaction"
              data-testid={`add-first-reaction-btn-${comment.id}`}
            >
            ...
            {showEmojiPicker === comment.id && (
              <>
                ...
                />
                {/* Emoji picker with enhanced sizing */}
                <div className={`absolute ${pickerPosition === 'left' ? 'right-full mr-2' : 'right-0'} top-10 bg-white border-2 border-gray-300 rounded-xl shadow-2xl z-[9999] p-5 min-w-[320px] max-w-[380px]`}>
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-base font-bold text-gray-900">Pick a reaction</span>
                    <button
                      onClick={() => setShowEmojiPicker(null)}
